...
export default function EditQuestionModal({ question, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire("Error", "Failed to update question: " + (err.response?.data?.message || err.message), "error");
    }
  };
        ...
        <>
          ...
          <details className="mt-8 border-t pt-4">
            ...
            <div className="mt-2 bg-gray-100 p-2 rounded text-xs overflow-auto max-h-64">
              ...
              {debugData?.data?.question_type && (
                <>
                  <h4 className="font-bold mt-4">Question Type Info:</h4>
                  <pre>{JSON.stringify(debugData.data.question_type, null, 2)}</pre>
                </>
              )}
            </div>
          </details>
